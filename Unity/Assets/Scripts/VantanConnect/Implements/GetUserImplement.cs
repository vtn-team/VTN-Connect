...
public class GetUserImplement
{
    ...
    {
        string request = String.Format("{0}/user/{1}", ProjectSettings.APIServerURI, userId);
        if (withLog)
        {
    ...
    {
        string request = String.Format("{0}/user/{1}", ProjectSettings.APIServerURI, userHash);
        if (withLog)
        {
            request += "?withLog=1&logCount" + logCount;
        }
